...
后续文本批次携带已接受的累计人物材料，以及当前批次前后各一段原文作为边界参考；相邻段不计入本批提交范围。原生视频提取沿用累计人物材料，并携带上一段大纲。连续动作、明确互称和已确认关系可用于关联具名人物与后续功能标签；相似外貌或服装本身不足以合并人物。姓名尚未确认与是否为同一人物分别记录，已建立的对应不因切段重新变成未知。

复刻保留关键视觉事件的作用：职业、权势、关系反差或剧情转折由哪些人物动作、空间、道具和反应表现。允许符合目标市场的等效画面，不把这些信息只留在旁白里，再用新增日常互动替换。规划、写作、分镜与审稿沿用同一要求，问题交现有局部 findings 和修订流程处理。
