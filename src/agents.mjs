...
export async function produceStoryboards(runDir) {
  ...
  const pendingEpisodes = Array.from({ length: m.episodes }, (_, index) => index + 1).filter((episode) => {
    const task = taskPath(runDir, `storyboard-ep-${ep(episode)}`), artifact = path.join(runDir, "storyboard", `ep-${ep(episode)}.md`);
    const prior = fs.existsSync(task) ? JSON.parse(readText(task)) : null;
    return !(prior?.state === "passed" && fs.existsSync(artifact) && prior.digest === sha(readText(artifact))
      && prior.contractDigest === contractDigest && prior.sourceScreenplayDigest === sha(readText(path.join(runDir, "screenplay", `ep-${ep(episode)}.md`))));
  });
  const concurrency = ["deepseek", "gpt"].includes(executionContractStatus(runDir).family) ? 8 : 1;
  let nextEpisode = 0, dispatchStopped = false;
  const takeEpisode = () => !dispatchStopped && nextEpisode < pendingEpisodes.length ? pendingEpisodes[nextEpisode++] : null;
  await mapConcurrent(Array.from({ length: Math.min(concurrency, pendingEpisodes.length) }, (_, index) => index + 1), concurrency, async (lane) => {
    let sessionNumber = 0, episode = takeEpisode();
    while (episode !== null) {
...
- 衔接显式化：同时空连续镜在备注格"连续性"行写明继承的视线、站位、道具；时空跳转先给建立镜再切近景。市场合同是硬约束：镜头中的人物、场景、道具、建筑、机构、服装和视觉提示必须属于目标国家，而非默认中国语境。`,customTools:[...tools,runChecks,submit],toolNames:["read_artifact","write_draft","run_checks","submit_storyboard"]});
    session=created.session;metrics=created.metrics;
    ...
    for(let count=0;count<5&&episode!==null&&!dispatchStopped;count++){
      const n=episode;assignedEpisodes.push(n);
      const task=taskPath(runDir,`storyboard-ep-${ep(n)}`);const artifact=path.join(runDir,"storyboard",`ep-${ep(n)}.md`);const source=readText(path.join(runDir,"screenplay",`ep-${ep(n)}.md`));const prior=fs.existsSync(task)?JSON.parse(readText(task)):null;active.episode=n;active.hasDraft=false;active.submitted=false;const repair=prior?.repairInstruction?`\n\n这是受约束的分镜修订任务。保留有效内容，只修下面的问题。\n现有分镜：\n${fs.existsSync(artifact)?readText(artifact):""}\n修订要求：\n${prior.repairInstruction}`:"";await promptWithWatchdog(session,metrics,`${sampleContext(m)}\n${planningBudgetGuidance(runDir,n,productionContract)}\n只制作第 ${n} 集，按 write_draft → run_checks → submit_storyboard 的顺序调用工具。\n市场与文化合同：\n${marketContract}\n\n生产合同：\n${contractText}\n\n剧本：\n${source}${storyboardSourceContext(runDir,n)}${repair}`,1800000);if(!active.submitted||!fs.existsSync(artifact))throw new Error(`storyboard agent did not submit episode ${n}`);
    ...
    outcome = "completed";
      ...
      session?.dispose();
      if (metrics) writeBatchMetrics(runDir, role, metrics, outcome, failure ? { error: failure.message } : {});
      appendAgentEvent(runDir, { type: "storyboard_batch_finished", ...audit, episodes: assignedEpisodes, outcome });
    }
    episode=takeEpisode();
    }
  });
