...
import {
  applyRepair,
  nextContinuityRepairAttempt,
  planningWatchdogMs,
  routeContinuityRejectToWriter,
  screenplayChecks,
} from "../src/agents.mjs";
...
function board(ep = 1) {
  const rows = [];
  for (let i = 1; i <= 12; i++) {
    const id = `ep${String(ep).padStart(2, "0")}-s${String(i).padStart(2, "0")}`;
    rows.push(
      `| ${id} | 画面 ${i} | 中：台词<br>EN: line | 固定机位 / 中景 | 人物：A<br>场景：S | 音效：环境声<br>功能：${i % 3 === 0 ? "情绪停留" : "对峙"} | 8 |`,
    );
  }
...
test("storyboard contract accepts a complete table", () =>
  assert.deepEqual(checkStoryboard(board()), []));
test("storyboard duration contract accepts 90 and 100 seconds and rejects 101", () => {
  ...
  assert.ok(
    checkStoryboard(boardWithDurations([9, 9, 9, 9, 9, 8, 8, 8, 8, 8, 8, 8])).some((error) =>
      error.includes("total duration 101"),
    ),
  );
});
test("storyboard contract rejects a missing fixed header", () =>
  assert.ok(checkStoryboard("| 镜头号 |\n|---|\n|1|").length));
...
test("repair plan blocks upstream and expands pair repairs", () => {
  assert.equal(
    repairPlan([{ episode: 3, severity: "P1", scope: "pair" }], 5).episodes.join(","),
    "3,4",
  );
...
test("applyRepair routes storyboard findings back to the Storyboard Agent task", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "tianshu-test-"));
  const { dir } = createRun(root, { title: "测试", episodes: 30, input: "x" });
  writeJson(path.join(dir, "canonical", "market.json"), {
    country: "United States",
    anchors: ["美国"],
    bannedContext: [],
  });
  writeText(path.join(dir, "canonical", "market-contract.md"), "美国市场");
  for (let episode = 1; episode <= 30; episode++) {
    writeText(
      path.join(dir, "screenplay", `ep-${String(episode).padStart(2, "0")}.md`),
      `# 第${episode}集`,
    );
    writeText(
      path.join(dir, "storyboard", `ep-${String(episode).padStart(2, "0")}.md`),
      board(episode),
    );
  }
  ...
  const finding = {
    ...
    scope: "local",
    category: "pace",
    evidence: "EP03 opening is static",
    reason: "No cold open",
    acceptance: "Conflict visible by second 3",
    repairInstruction: "Open on the conflict",
    preserve: ["plot"],
    doNotChange: ["screenplay"],
    disposition: "repair",
  };
  const plan = {
    stage: "storyboard",
    action: "repair",
    cycle: 1,
    artifactDigest: stageArtifactDigest(dir, "storyboard"),
    contractDigest,
    episodes: [3],
    findings: [finding],
  };
...
test("continuity rejection becomes a durable bounded Writer repair task across resume", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "tianshu-continuity-route-")),
    { dir } = createRun(root, { title: "连续性路由", episodes: 30, input: "x" }),
    taskFile = path.join(dir, "tasks", "screenplay-ep-03.json"),
    error = Object.assign(new Error("rejected"), {
      code: "CONTINUITY_REJECTED",
      review: { reason: "钥匙状态冲突" },
    });
  routeContinuityRejectToWriter(dir, { episode: 3, error, taskFile, attempt: 1, maxAttempts: 2 });
  const task = JSON.parse(fs.readFileSync(taskFile));
  assert.equal(task.state, "stale");
  assert.match(task.repairInstruction, /钥匙状态冲突/);
  assert.match(task.repairInstruction, /大纲规定的事件在正文漏写，应补回正文/);
  assert.match(task.repairInstruction, /不得为了通过连续性检查将其删掉、推迟到后集或标成未发生/);
  assert.equal(nextContinuityRepairAttempt(JSON.parse(fs.readFileSync(taskFile))), 2);
  routeContinuityRejectToWriter(dir, {
    episode: 3,
    error,
    taskFile,
    prior: task,
  ...
  const resumed = JSON.parse(fs.readFileSync(taskFile));
  assert.equal(nextContinuityRepairAttempt(resumed), 3);
  assert.throws(
    () =>
      routeContinuityRejectToWriter(dir, {
        episode: 3,
  ...
  assert.equal(loadManifest(dir).state, "needs_human_review");
});
test("delivery requires every checked storyboard", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "tianshu-test-"));
  const { dir } = createRun(root, { title: "测试", episodes: 30, input: "x" });
  const contractDigest = productionContractDigest(loadProductionContract(dir));
  writeText(path.join(dir, "canonical", "characters.md"), "A (30): lead.");
  writeJson(path.join(dir, "canonical", "ledger.json"), { names: ["A"], facts: ["x", "y", "z"] });
  writeText(path.join(dir, "canonical", "market-contract.md"), "US market");
  for (let ep = 1; ep <= 30; ep++) {
    const screenplay = `# 第${ep}集｜已审核集名${ep}`;
    const storyboard = board(ep);
    writeText(path.join(dir, "screenplay", `ep-${String(ep).padStart(2, "0")}.md`), screenplay);
    writeText(path.join(dir, "storyboard", `ep-${String(ep).padStart(2, "0")}.md`), storyboard);
    writeJson(path.join(dir, "tasks", `storyboard-ep-${String(ep).padStart(2, "0")}.json`), {
      state: "passed",
      digest: sha(`${storyboard.trimEnd()}\n`),
      contractDigest,
      sourceScreenplayDigest: sha(`${screenplay}\n`),
    });
  }
...
test("delivery headings normalize suffixes and preserve all prose and table bytes", () => {
  ...
  for (const heading of [
    "# 第10集｜需要一把椅子证明的王冠 分镜表",
    "## 第 10 集 | 需要一把椅子证明的王冠 · 分镜表",
    "# 第10集｜需要一把椅子证明的王冠｜分镜剧本",
    "# 第10集｜需要一把椅子证明的王冠",
  ]) {
    const result = renderDeliveryMarkdown(
      manifest,
      [`${heading}${body}`],
      ["# 第10集｜不替代已有集名"],
    );
  ...
  assert.equal(
    renderDeliveryMarkdown(manifest, [`# 第10集｜开场（172.0s）${body}`]),
    `${summary}# 第 10 集｜开场｜预计 8 秒${body}`,
  );
});
test("delivery headings recover generic and missing titles from the same episode screenplay", () => {
  ...
  for (const heading of ["# 分镜剧本", "# 第21集", "# 第21集｜分镜表", "# 第21集｜分镜剧本"]) {
    assert.equal(
      renderDeliveryMarkdown(manifest, [`${heading}${body}`], ["# 第21集｜日翼晶体粉"]),
      `${summary}# 第 21 集｜日翼晶体粉｜预计 8 秒${body}`,
    );
  }
  const missing = body.trimStart();
  assert.equal(
    renderDeliveryMarkdown(manifest, [missing], ["# 第21集｜日翼晶体粉"]),
    `${summary}# 第 21 集｜日翼晶体粉｜预计 8 秒\n\n${missing}`,
  );
  assert.equal(
    renderDeliveryMarkdown(manifest, [`# 分镜剧本${body}`], ["# 第22集｜其它集名"]),
    `${summary}# 第 21 集｜预计 8 秒${body}`,
  );
  assert.throws(
    () => renderDeliveryMarkdown({ episodes: 2 }, ["# 已有集名", "# 分镜表"]),
    /no storyboard tables/,
  );
  assert.throws(
    () => renderDeliveryMarkdown({ episodes: 1 }, ["# 第41集｜旧稿（172.0s）\n\n| 01 | 画面 |"]),
    /no storyboard tables/,
  );
});
...
test("planner market submission must match the requested country", () => {
  ...
  const valid = {
    country: "United States",
    setting: "New York contemporary auction world",
    characterNaming: "Natural contemporary American names",
    socialContext: "US family wealth and auction institutions",
    culturalAnchors: ["New York", "estate sale"],
  };
...
test("market digest binds both machine-readable and Markdown contracts", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tianshu-market-digest-"));
  fs.mkdirSync(path.join(dir, "canonical"), { recursive: true });
  writeJson(path.join(dir, "canonical", "market.json"), { country: "United States" });
  writeText(path.join(dir, "canonical", "market-contract.md"), "US");
  const before = marketArtifactDigest(dir);
  writeJson(path.join(dir, "canonical", "market.json"), { country: "Canada" });
  assert.notEqual(marketArtifactDigest(dir), before);
});
test("screenplay contract rejects dialogue that cannot fit a 120-second episode", () => {
  ...
  assert.ok(
    screenplayChecks(screenplay, 1, { anchors: [], bannedContext: [] }).some((error) =>
      error.includes("英文对白过长"),
    ),
  );
});
...
test("fixed-entity contract rejects a known first name with a drifting surname", () => {
  ...
  assert.ok(
    screenplayChecks(
      `# 第1集｜EP01\n\n## 场景一\n\nRowan Hale 出场。\nMAYA（中）：成交。\nMAYA（EN）：Deal.\n\n${"剧情动作。".repeat(180)}\n\n## 【本集钩子】\n局面改变。\n\n## 【连续性检查】\n状态已记录。`,
      1,
      { anchors: [], bannedContext: [] },
      ["Rowan Kade"],
    ).some((error) => error.includes("Rowan Hale")),
  );
  assert.ok(
    !screenplayChecks(
      ...
      { anchors: [], bannedContext: [] },
      ["Rowan Kade"],
    ).some((error) => error.includes("姓名漂移")),
  );
  assert.ok(
    !screenplayChecks(
...
test("fixed-entity contract derives people from the character bible, not team or company names", () => {
  assert.deepEqual(
    canonicalPersonNames("Tessa Ward (31): analyst. Fixed entities: Windsor Foundry.", [
      "Tessa Ward",
      "Windsor Foundry",
    ]),
    ["Tessa Ward"],
...
test("canonical names use shared character-entry prefixes without harvesting annotations or locations", () => {
  const characters =
    "Ava Miller（源字幕偶作 eva、Evan）：护理员。\nKael（源字幕偶作 Kyle，待确认）：黑龙王。\nKnox / Nox（同一幼龙，拼写并存）：黑鳞小王子。\nRoyal Palace Baby Care Hall / 育婴殿：场景。\nCradle Holy Seal 摇篮圣印：道具。\n附注提到 Hidden Person (31)，不构成人物条目。";
  const ledgerNames = [
    "Ava Miller（源字幕偶作 eva、Evan；保留原拼写）",
    "Kael（源字幕偶作 Kyle，待确认）",
    "Knox / Nox（同一幼龙，黑鳞小王子）",
    "Kyle",
    "Hidden Person",
    "Royal Palace Baby Care Hall / 育婴殿",
    "Cradle Holy Seal 摇篮圣印",
  ];
  assert.deepEqual(canonicalPersonNames(characters, ledgerNames), [
    "Ava Miller",
    "Kael",
    "Knox",
    "Nox",
...
test("screenplay scene limit rejects nine numbered Writer scenes when the contract allows eight", () => {
  ...
  for (const title of ["场", "场 ", "场景", "SCENE "]) {
    ...
    assert.ok(
      screenplayChecks(markdown, 9, market, [], contract).includes("场景过多 9（上限 8）"),
      title,
    );
  }
});
...
test("screenplay scene limit accepts eight numbered scenes and ignores scene words in body text", () => {
  ...
  const errors = screenplayChecks(markdown, 9, { anchors: [], bannedContext: [] }, [], {
    screenplay: { englishDialogueWordLimit: 1000, maxScenes: 8 },
  });
