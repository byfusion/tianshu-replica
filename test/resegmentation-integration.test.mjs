...
import * as episodeMap from "../src/episode-map.mjs";
import * as replication from "../src/replication.mjs";
import * as market from "../src/market.mjs";
import { loadProductionContract, episodeDurationPolicy } from "../src/production-contract.mjs";
import { appendRunMetrics } from "../src/metrics.mjs";
import { canonicalReviewContext, episodePayload, planningPayload } from "../src/semantic-review.mjs";
import { Type, Compile, defineTool, readText, writeText, writeJson } from "../src/experiments/lib.mjs";
...
function fixture(t) {
  ...
  const input = "目标市场：中国。保留全部源事件，长源段按冲突和尾钩按60–90秒目标、75秒常态、100秒硬上限重分输出集。";
  const { dir } = core.createRun(root, { title: "长源段重分集离线验证", input, sourceMaterials: materials, sourceTotalEpisodes: 32 });
  const map = Array.from({ length: 31 }, (_, index) => ({
    sourceEpisodes: [index + 1], startEvent: `源事件${index + 1}开始。`, endEvent: `源事件${index + 1}结束。`, targetSeconds: 90,
  }));
  map.push(
    { sourceEpisodes: [32], startEvent: lastSource.firstStart, endEvent: lastSource.firstEnd, targetSeconds: 78 },
    { sourceEpisodes: [32], startEvent: lastSource.secondStart, endEvent: lastSource.secondEnd, targetSeconds: 88 },
  );
  const bundle = {
    market: { ...market.inferMarketIntent(input), setting: "中国街区的旧店与修理铺，原有商业与日常生活环境保持对应。", characterNaming: "保留林夏、周舟这两个已确认姓名，不给未确认来客新增身份。", socialContext: "中国街坊相互协作修复旧店，不新增超出原素材的制度关系。", culturalAnchors: ["街坊旧店", "修理铺"] },
    ...
    continuityContract: "保留已经发生的源事件与人物认知，按输出集逐集推进；后续事件未发生时不提前写成当前状态，未确认身份仍未知。".repeat(6),
    outline: map.map((part, index) => `输出第${index + 1}集：${part.startEvent}依照原有行动与因果推进，不重复上一部分结局，不提前兑现下一部分事件。${part.endEvent}`),
    episodeMap: map,
  };
...
function offlinePlanner(action) {
  ...
  let sessions = 0;
  const run = vm.runInNewContext(`(${generation})`, {
    fs, path, Type, Compile, defineTool, readText, writeText, writeJson,
    ...core, ...market, ...replication, ...episodeMap, ...agents,
    loadProductionContract, episodeDurationPolicy, appendRunMetrics,
    createPiExperimentSession: async (options) => {
      sessions += 1;
      return { session: { options, dispose() {} }, metrics: { prompts: 1, assistantMessages: 0, usage: [] } };
    },
    promptWithWatchdog: async (session, _metrics, prompt) => action(session.options, async (name, args) => {
      const tool = session.options.customTools.find((item) => item.name === name);
      assert.ok(tool, `missing ${name}`);
      assert.ok(Compile(tool.parameters).Check(args), "offline submission must satisfy the real tool schema");
...
function offlinePlan(planner, reviewStage) {
  return vm.runInNewContext(`(${agents.plan.toString()})`, {
    ...core, ...agents, generatePlanningBundle: planner.run, reviewStage,
  });
}
...
test("a long source can split into a later output episode and all production roles receive its actual source mapping", async (t) => {
  ...
  for (const context of [
    replication.replicationWriterContext(f.dir, 33), agents.storyboardSourceContext(f.dir, 33),
    episodePayload(f.dir, "screenplay", 33), episodePayload(f.dir, "storyboard", 33),
  ]) {
...
test("a local planning patch preserves the approved split map and output count", async (t) => {
  const f = fixture(t);
  const initial = offlinePlanner(async (_settings, invoke) => assert.match((await invoke("submit_planning_bundle", f.bundle)).content[0].text, /^ACCEPTED/));
  ...
  const patch = offlinePlanner(async (_settings, invoke, prompt) => {
    ...
    const result = await invoke("submit_planning_patch", { replacements: [{
      file: "acts.md", old_text: "需要精修的阶段句。", new_text: "本阶段只处理已经发生的事件，来客身份仍未知。",
    }] });
    assert.match(result.content[0].text, /^ACCEPTED/);
  });
  await offlinePlan(patch, async () => ({ plan: { action: "pass" } }))(f.dir, { operatorNote: "仅精修 acts.md 的指定句，保留现有分集。" });
...
test("review reception failure resumes the completed mapped plan without another Planner call", async (t) => {
  const f = fixture(t);
  const planner = offlinePlanner(async (_settings, invoke) => assert.match((await invoke("submit_planning_bundle", f.bundle)).content[0].text, /^ACCEPTED/));
  let reviewCalls = 0;
