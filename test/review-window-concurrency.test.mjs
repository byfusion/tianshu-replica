...
function fixture(t, { episodes = 43, provider = "deepseek", legacyUnbound = false, stage = "screenplay", candidates = [] } = {}) {
  ...
  const reviewStage = vm.runInNewContext(`${source}\nreviewStage`, {
    ...
    createPiExperimentSession: async (options) => {
      if (options.role === state.failSessionRole) throw new Error("offline review session creation failure");
  ...
  return {
    ...
    events: () => fs.existsSync(path.join(runDir, "events.jsonl"))
      ? readText(path.join(runDir, "events.jsonl")).trim().split("\n").map((line) => JSON.parse(line)) : [],
    assertUncommitted() {
...
test("DeepSeek and GPT run eight independent windows and wait for all windows before one final review", async (t) => {
  for (const provider of ["deepseek", "openai-codex"]) {
    for (const stage of ["screenplay", "storyboard"]) {
      ...
      assert.equal(f.state.active, 8);
      const firstWindow = f.windows()[0], tailWindow = f.windows()[8];
      assert.equal(tailWindow.to - tailWindow.from + 1, 3);
      assert.equal(JSON.stringify(firstWindow.options.customTools[0].parameters), JSON.stringify(tailWindow.options.customTools[0].parameters), "short final windows retain the same tool prefix");
      assert.equal(firstWindow.options.systemPrompt, tailWindow.options.systemPrompt);
      assert.equal(firstWindow.prompt.split("Review window:\n")[0], tailWindow.prompt.split("Review window:\n")[0], "shared canonical context precedes window-specific text");
      const incomplete = await tailWindow.options.customTools[0].execute("incomplete-tail", { episodeSummaries: [{ episode: tailWindow.from }] });
      assert.match(incomplete.content[0].text, /^REJECTED episode summaries must cover the review window in order/);
      ...
      assert.deepEqual(readJson(path.join(f.runDir, `reviews/${stage}-final.json`)).windowReviews.map(({ from }) => from), [1, 6, 11, 16, 21, 26, 31, 36, 41]);
      ...
      for (const event of events) {
        assert.deepEqual(Object.keys(event).sort(), ["at", "cycle", "from", "outcome", "role", "run", "stage", "to", "type"]);
        ...
        if (event.type === "review_window_started") { active++; peak = Math.max(peak, active); assert.equal(event.outcome, "running"); }
        else { active--; assert.equal(event.outcome, "completed"); }
      }
      assert.equal(peak, 8);
      assert.equal(active, 0);
      if (provider === "openai-codex") {
        assert.equal(f.series().options.thinkingLevel, undefined, "GPT keeps its adapter reasoning policy");
...
test("window failure drains in-flight reviews without final submission and resumes unchanged successful checkpoints", async (t) => {
  ...
  const newStarts = f.events().slice(failedEvents.length).filter((event) => event.type === "review_window_started");
  assert.deepEqual(newStarts.map((event) => event.from), [11, 41], "cached successful windows do not emit new execution starts");
});
...
test("a failed window session creation records failure without a final review", async (t) => {
  ...
  assert.deepEqual(f.events().map(({ type, outcome }) => [type, outcome]), [
    ["review_window_started", "running"], ["review_window_finished", "failed"],
  ]);
  assert.equal(f.state.seriesCalls, 0);
  assert.equal(fs.existsSync(path.join(f.runDir, "reviews/checkpoints/screenplay-window-1-1-cycle-2.json")), false);
  f.assertUncommitted();
});
