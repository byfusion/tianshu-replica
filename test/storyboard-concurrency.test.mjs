...
function storyboardRun({ episodes = 45, provider = "deepseek", legacyUnbound = false, seeded = [], stale = [], failEpisode = null, failSessionRole = null, omitSubmit = null, omitWrite = null, beforeEpisode = async () => tick(), markdownForEpisode = (episode, role) => `STORYBOARD EP${episode} FROM ${role}`, sourceForEpisode = (episode) => `SOURCE EP${episode}`, checkDraft = () => [] } = {}) {
  const root = "/offline-storyboard", files = new Map(), workDirs = [], calls = [], batchesDone = [], states = [], events = [];
  ...
  const run = vm.runInNewContext(`(${body})`, {
    ...
    appendAgentEvent: (runDir, event) => { assert.equal(runDir, root); events.push({ at: new Date().toISOString(), ...event }); },
    ...
    createPiExperimentSession: async ({ role, customTools }) => {
      if (role === failSessionRole) throw new Error("offline session creation failure");
    ...
    promptWithWatchdog: async (session, _metrics, prompt) => {
      ...
      try {
        await beforeEpisode(episode, session.role);
  ...
  return { run: () => run(root), files, calls, workDirs, batchesDone, states, events, manifest, peak: () => peakSessions, active: () => activeSessions };
}

test("DeepSeek and GPT storyboard lanes reuse sessions for at most five episodes and retain passed tasks", async () => {
  for (const provider of ["deepseek", "openai-codex"]) {
    const state = storyboardRun({ seeded: [2], provider });
    await state.run();
    ...
    assert.ok(state.calls.some(({ role }) => state.calls.filter((item) => item.role === role).length === 5));
    assert.ok(state.calls.some(({ role }) => /-session-2$/.test(role)), "lanes rotate a full session before taking more work");
    ...
    assert.equal(state.events.filter((event) => event.type === "storyboard_batch_started").length, state.workDirs.length);
    assert.equal(state.events.filter((event) => event.type === "storyboard_batch_finished").length, state.workDirs.length);
    let active = 0, peak = 0;
    for (const event of state.events) {
      assert.deepEqual(Object.keys(event).sort(), ["at", "episodes", "lane", "outcome", "role", "run", "sessionNumber", "type"]);
      assert.equal(event.run, "offline-storyboard");
      ...
      const assigned = state.calls.filter((call) => call.role === event.role).map((call) => call.episode);
      if (event.type === "storyboard_batch_started") {
        active++; peak = Math.max(peak, active); assert.equal(event.outcome, "running");
        assert.deepEqual(Array.from(event.episodes), assigned.slice(0, 1));
      } else {
        active--; assert.equal(event.outcome, "completed");
...
test("thirty episodes start eight lanes and free lanes keep taking episodes while EP1 remains slow", async () => {
  const gates = new Map();
  const state = storyboardRun({ episodes: 30, provider: "openai-codex", beforeEpisode: (episode) => new Promise((resolve) => gates.set(episode, resolve)) });
  const pending = state.run();
  await tick();
  assert.deepEqual(state.calls.map(({ episode }) => episode), [1, 2, 3, 4, 5, 6, 7, 8]);
  assert.equal(state.active(), 8);
  gates.get(8)();
  await tick();
  assert.equal(state.calls.at(-1).episode, 9, "a free lane immediately takes the next episode");
  assert.equal(state.calls.at(-1).role, state.calls.find(({ episode }) => episode === 8).role, "the lane keeps its warm session");
  const released = new Set([8]);
  for (let turn = 0; turn < 30 && state.calls.length < 30; turn++) {
    for (const { episode } of state.calls) if (episode !== 1 && !released.has(episode)) {
      released.add(episode); gates.get(episode)();
    }
    await tick();
  }
  ...
  assert.equal(state.files.has("/offline-storyboard/storyboard/ep-01.md"), false);
...
test("passed storyboards are filtered before any sessions are created, including a fully resumed stage", async () => {
  const seeded = Array.from({ length: 30 }, (_, index) => index + 1);
  const complete = storyboardRun({ episodes: 30, provider: "openai-codex", seeded });
  await complete.run();
  ...
  const partial = storyboardRun({ episodes: 30, provider: "openai-codex", seeded: seeded.filter((episode) => ![2, 17, 29].includes(episode)) });
  await partial.run();
  assert.equal(partial.workDirs.length, 3);
  assert.equal(partial.peak(), 3);
  assert.deepEqual(partial.calls.map(({ episode }) => episode), [2, 17, 29]);
});

test("a failed episode stops new dispatch, awaits already-started episodes and leaves the phase resumable", async () => {
  const state = storyboardRun({ provider: "openai-codex", failEpisode: 1 });
  ...
  assert.equal(finished.find((event) => event.outcome === "failed").role, "storyboard-lane-1-session-1");
});

test("a failed storyboard session creation records a failed batch finish", async () => {
  const state = storyboardRun({ episodes: 1, provider: "openai-codex", failSessionRole: "storyboard-lane-1-session-1" });
  await assert.rejects(state.run(), /offline session creation failure/);
  assert.deepEqual(state.events.map(({ type, outcome }) => [type, outcome]), [
    ["storyboard_batch_started", "running"], ["storyboard_batch_finished", "failed"],
  ]);
  assert.equal(state.active(), 0);
...
test("Kimi storyboard batches keep one active session", async () => {
...
test("a prior episode draft cannot be checked or submitted as the next storyboard", async () => {
  const state = storyboardRun({ episodes: 2, provider: "kimi-coding", omitWrite: 2 });
  ...
  assert.match(state.files.get("/offline-storyboard/work/storyboard-lane-1-session-1/draft.md"), /^# 第 1 集/);
  assert.equal(state.manifest.state, "storyboard_producing");
});
